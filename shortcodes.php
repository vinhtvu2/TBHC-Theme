			...
			foreach ($slide_order as $s) {
				...
				if ( ($s !== '') && ($s !== NULL) ) {
					...
					if ($slide_content_type[$s] == 'image') {
						...
						$output .= '<div class="centerpiece_single img" style="background-image:url(\''.$slide_image_url[0].'\');"';
						$output .= '/>';

						if($slide_display_tit[$s] == 'on'){
							$rgba = hex_and_opacity_to_rgba($slide_tit_bg_color[$s], $slide_tit_opacity[$s]);
							$output .= '<div style="top:'.$slide_tit_off_top[$s].';left:'.$slide_tit_off_left[$s].';font-size:'.$slide_tit_font_sz[$s].';color:'.$slide_tit_font_col[$s].';background-color:rgba('.$rgba.');">'.$slide_title[$s].'</div>';
						}
