...
function sc_person_profile_grid($atts) {
	...
	?><div class="person-profile-grid" data-url="<?=admin_url( 'admin-ajax.php' )?>">
		...
		foreach($people as $person) {
			...
			$image = wp_get_attachment_image(get_post_thumbnail_id($person->ID), 'profile-grid-image' );
			$image_url = get_featured_image_url($person->ID);
			$link = ($person->post_content != '') ? True : False;
			/*if( ($count % $row_size) == 0) {
				if($count > 0) {
		...
		<div class="col-md-2 col-sm-2 person-profile-wrap">
			<? if($link) {?><a href="<?=esc_attr(get_permalink($person->ID))?>"><? } ?>
				<img src="<?=esc_attr($image ? $image : get_bloginfo('stylesheet_directory').'/static/img/no-photo.jpg')?>" />
...
function sc_events_widget() {
	...
	<p class="screen-only"><a href="https://events.ucf.edu/calendar/2862/the-burnett-honors-college/upcoming/" class="events_morelink">More Events</a></p>';
}
