...
function getProfilesForGrid(inp1, inp2){ // moved out person-profile-grid to new var, added rowSize to post data
	var ele = $(".person-profile-grid");
	var ajaxUrl = ele.data("url");
	var reqGrp = ele.data("group");
	var regGrp2 = ele.data("group2");
	var shwOrgs = ele.data("shwgrp");
	var oprtr = ele.data("oprtr");	
	var jn = ele.data("jn");
	var shwOptAll = ele.data("allopt");
	var shwOptAll2 = ele.data("allopt2");	
	var rowSize = ele.data("rowsize");
	var data = {
...
function getSpotsForGrid(inp1, inp2){
	var ajaxUrl = $(".spotlight-grid").data("url");
	var reqGrp = $(".spotlight-grid").data("group");
	var reqGrp2 = $(".spotlight-grid").data("group2");
	var shwOrgs = $(".spotlight-grid").data("shwgrp");
	var oprtr = $(".spotlight-grid").data("oprtr");	
	var jn = $(".spotlight-grid").data("jn");
	var shwOptAll = $(".spotlight-grid").data("allopt");
	var shwOptAll2 = $(".spotlight-grid").data("allopt2");	
	var data = {
		action : 'get_spots_from_event_group',
		join : jn,
		event_groups: inp1,
		dd_event_groups: reqGrp,
	};
...
function stickyHeadFix(){
	...
	headWrp.on('affixed.bs.affix', function(){ toPrep.prepend("<div id='fakeNav' style='height:" + headWrp.height() + "px;display:block;'></div>"); })
	headWrp.on('affix-top.bs.affix', function(){ $('#fakeNav').detach(); })
}
(function() {
